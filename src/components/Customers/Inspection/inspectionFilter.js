...
import miniCalendarIcon from '/src/images/icons/agent_req_calendar.png';
...
import {CustomSingleDatePicker} from '../../common/Calendar/forDateAndTimePicking';
// import { changeDateFormat } from "../../../../../utils/formatDate";

export const InspectionFilter = ({setUrl, user, url}) => {
  ...
  console.log(date?.from, dateTo.getFullYear() > 2000);
  // const user =
  ...
  const sign_up_date = new Date(user.sign_up_time);
  console.log(dateFrom, user.sign_up_time);
    ...
    setUrl(
      `?${filter_param}${
        toBeFiltered.inspection_type.length < 2
          ? `${
              filter_param ? '&' : ''
            }inspection_type=${toBeFiltered.inspection_type[0]?.toLowerCase()}`
          : ''
      }`
    );
          ...
          <VStack spacing="7px" w="full">
            ...
            <VStack px="26px" w="full" spacing="5px">
              ...
              {/* <CustomSingleDatePicker
                handleSelectedDate={() => handleChange("", "to")}
                mainDate={date.to}
              /> */}
              {dateError && (
