...
import {makeUnitPrivate} from 'apis/listings';
import {descStyle} from '../../listingMoreOptionsDrawer/screens/mapViewDrawer';

export const UnitVisibility = ({isOpen, onClose, handleBack, unitId, isPrivate, refetch}) => {
  const toast = useToast();
  const status = isPrivate ? 'False' : 'True';
  const mutation = useMutation(formData => makeUnitPrivate(unitId, status), {
    onSuccess: async res => {
      toast({
        status: 'success',
        position: 'top-right',
        duration: 3000,
        title: isPrivate ? 'Unit has been made public' : 'Unit has been privatized',
      });
      refetch();
      onClose();
    },
        ...
        >
          <Flex align={'center'} gap="5px">
            ...
            <Heading fontSize="18.9px" fontWeight="700">
              {isPrivate ? 'Make Unit Public' : 'Make Unit Private'}
            </Heading>
          </Flex>
          <HStack spacing="15px">
            <VStack
          ...
          {!isPrivate ? (
                ...
                <Stack spacing="32px" mt="40px">
                  <HStack spacing={'12px'} align="center">
                    ...
                    <Box>
                      ...
                      <Text {...descStyle}>
                        By making a unit private, it's kept off public view and out of sight from
                  ...
                  <HStack spacing={'12px'} align="center">
                    ...
                    <Box>
                      ...
                      <Text {...descStyle}>
                        ...
                        access to the unit in their portfolio section.
                      </Text>
                    </Box>
                  </HStack>
                </Stack>
            ...
            >
              ...
              <Text textAlign={'center'} fontSize={14} color={'#606060'}>
                Setting this unit to public will grant access to all users for a better sharing
                experience
              </Text>
              <HStack gap={4}>
                <Button
