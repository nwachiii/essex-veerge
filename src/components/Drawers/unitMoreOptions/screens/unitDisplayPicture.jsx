...
import {fetchUnitDisplayImages, updateUnitDisplayPicture} from 'apis/listings';
...
export const UnitDisplayPicture = ({isOpen, onClose, refetch, bundleId, unitInfo}) => {
  const GET_DISPLAY_IMAGE = useQuery(
    ['display-images'],
    () => fetchUnitDisplayImages(bundleId),
    {
      enabled: !!isOpen,
    }
  );
  ...
  const mutation = useMutation({
    mutationFn: formData => updateUnitDisplayPicture(bundleId, formData),
    onSuccess: async res => {
  ...
  const CURRENT_DISPLAY_PICTURE = GET_DISPLAY_IMAGE.data?.data?.profile;

  const itemsToShow = 5;
  const breakPoints = [{width: 1, itemsToShow}];

  const [selectedImage, setSelectedImage] = useState(
    CURRENT_DISPLAY_PICTURE || unitInfo?.photos[0]?.photo
  );

  useEffect(() => {
    if (CURRENT_DISPLAY_PICTURE) {
      setSelectedImage(CURRENT_DISPLAY_PICTURE);
    } 
  }, [CURRENT_DISPLAY_PICTURE]);

  console.log(GET_DISPLAY_IMAGE)
                ...
                <HStack spacing={2} overflowX="auto" mt={0}>
                  {GET_DISPLAY_IMAGE.isLoading || GET_DISPLAY_IMAGE.isPending ? (
                    ...
                    <Stack w="90%">
                      ...
                      >
                        {unitInfo?.photos?.map((item, idx) => (
                          <AnimatePresence key={idx}>
                            <Stack position={'relative'}>
                              {CURRENT_DISPLAY_PICTURE === item?.photo && (
                                <Box
                                  top="3px"
                                  right="3px"
                                  position="absolute"
                              ...
                              <Image
                                ...
                                border={
                                  selectedImage === item?.photo ? '3px solid blue' : '1px solid lightgray'
                                }
                              />
                            </Stack>
                          </AnimatePresence>
