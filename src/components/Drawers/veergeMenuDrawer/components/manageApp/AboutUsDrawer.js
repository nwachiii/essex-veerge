...
import {updateAboutUsUrl} from 'apis/settings';
...
export const AboutUsDrawer = ({url, refetch}) => {
  const [url_value, set_url_value] = useState(url || '');
  const toast = useToast();
  const disclosure = useDisclosure();

  const mutation = useMutation(formData => updateAboutUsUrl(formData), {
    onSuccess: async res => {    
      toast({
        render: () => (
          <MatadorCustomToast description={`About us url ${url ? `updated` : `added`}!`} />
        ),
        duration: 4000,
        isClosable: true,
        position: 'top-right',
  ...
  const update_about_url = async url => {
    mutation.mutate({about: url_value});
  };
        ...
        <DrawerContent mt="65px" p="0px" maxW={`400px`}>
          ...
          <DrawerFooter p={`30px 28px`}>
            <Button
              ...
              _active={{bg: `#0D0D0D`}}
              onClick={update_about_url}
              isDisabled={mutation?.isLoading || !isValidURL(url_value)}
              rounded='72px'
            >
