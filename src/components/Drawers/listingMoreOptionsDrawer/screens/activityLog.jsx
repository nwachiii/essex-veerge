...
import {activityLogListings} from 'apis/account';
import ActivityLogList from '../../activitylogDrawer/ActivityLogList';

const ActivityLog = ({isOpen, onClose, id}) => {
  const {data, refetch, isLoading, isError, error} = useQuery(['activity-log', id], () =>
    activityLogListings({id})
  );
    ...
    <Drawer isOpen={isOpen} onClose={onClose}>
      ...
      <DrawerContent position="relative" zIndex={100} mt="65.12px" minW="450px" bg="#fff" p="0px">
        ...
        <DrawerBody marginTop="1rem" padding="0">
          <ActivityLogList data={data} isLoading={isLoading} error={error} isError={isError} />
        </DrawerBody>
      </DrawerContent>
    </Drawer>
  );
