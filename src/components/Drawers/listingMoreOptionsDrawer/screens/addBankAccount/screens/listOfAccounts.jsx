...
import {fetchProjectBankAccounts} from 'apis/settings';

const ListOfAccounts = ({handleScreen, projectId, setAccountId, mainScreenNav}) => {
  ...
  const {data, isLoading, isError, error} = useQuery(['project bank accounts'], () =>
    fetchProjectBankAccounts(projectId)
  );
  ...
  console.log({data});
  const bankAccountsArray = data?.data?.results;
  return (
    <>
      <HStack
      ...
      <DrawerBody p="0px" px="20px">
        {isLoading ? (
          ...
          <Stack
            ...
            py="24px"
          >
            {bankAccountsArray?.map((item, idx) => (
              <Flex key={idx} w="full" justify="space-between" pl="24px" pr="16.53px">
                ...
                <Stack textTransform="capitalize" spacing="10px" color="#606060">
