...
import {removeABankAccount} from 'apis/settings';

const ConfirmRemovingAnAccount = ({accoundId, handleScreen}) => {
  const toast = useToast();
  const queryClient = useQueryClient();
  const {mutate, isLoading} = useMutation(
    values => {
      return removeABankAccount(values, accoundId);
    },
    {
      onSuccess: res => {
        queryClient.refetchQueries(['bank accounts']);
        toast({
          status: 'success',
          position: 'top-right',
  ...
  return (
    <>
      <HStack
        ...
        bg="#F5F5F5"
      >
        <Image boxSize="24px" cursor="pointer" src={backIcon.src} alt="back icon" />
      </HStack>
