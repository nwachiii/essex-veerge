...
import PropertyInfo from './property_details';
import backArrow from '/src/images/icons/back-arrow.png';
// import InspectionFilter from "../../../../components/Customers/Inspection/InspectionFilter";

export const InspectionDetails = ({data, user, setAddedParam}) => {
  ...
  console.log(paginate(data, 10)[+count - 1]);
  return (
    <VStack
      w="full"
      //  maxW="888px"
    ...
    >
      <HStack w="100%" justify="space-between">
        ...
        <HStack>
          ...
          {/* <InspectionFilter user={user} setUrl={setAddedParam} /> */}
        </HStack>
      </HStack>
      <VStack w="full" spacing="24px" bg="#FFFFFF" borderRadius="16px" px="27px" py="27px">
        {paginate(data, 10)[~~count - 1].map((item, idx) => (
          <PropertyInfo key={idx} data={item} />
        ))}
      </VStack>
    </VStack>
  );
